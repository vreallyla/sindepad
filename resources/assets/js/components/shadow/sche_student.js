if ($('Body').find('#sche-student').length > 0) {
    $(function () {
        const targetObj = $('#sche-student'),
            ...
            rowAvail = [10, 30, 50], catAvail = ['Sudah Diatur', 'Belum Diatur'],
            targetPage = targetObj.find('.paginate'),
            loadingMagnify = targetObj.find('.loading'),
            tableLabel = targetObj.find('.table-register'),
            urlPage = '/simdepad/shadow/schedules/student',
            urlApi = '/api/v1/shadow/set-schedules/';
            ...
            getListAsync = function () {
                ...
                axios.get(urlApi + 'list', {
                    ...
                    for (let i = 0; i < datalist.length; i++) {
                        if (datalist[i].schedule) {
                            let targetSelSearch = targetObj.find('tbody').children().eq(i).find('.add'),
                                selValSearch = targetSelSearch.find('option[value=' + (datalist[i].schedule ? datalist[i].schedule.key : '') + ']');
                            targetSelSearch.find('.filter-option-inner-inner').text(selValSearch.text());
                            selValSearch.parent().val(datalist[i].schedule ? datalist[i].schedule.key : '');
                        }
                    }
        ...
        $(window).ready(function () {
            ...
            if (data.cat !== categoryTarget.val() && findInArray(catAvail, data.cat)) {
        ...
        targetTable.on('click', '.btn-danger', function () {
            ...
            loadPart.show();
            axios.put(urlApi + 'update', dataStu)
                .then(res => {
                    loadPart.hide();
